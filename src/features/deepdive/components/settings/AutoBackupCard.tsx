...
import { useAutoBackup } from '@/features/deepdive/hooks/useAutoBackup'
      ...
      <Card>
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            ...
            DeepDive in Chrome / Edge / Brave to enable automatic local snapshots.
          </p>
        </CardContent>
      </Card>
    )
      ...
      <CardContent className="space-y-4">
        {!backup.handle ? (
          <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
            Pick a local folder. DeepDive will save a JSON snapshot of all <code>pt_</code> data on
            a schedule. Choose a synced folder (Dropbox, OneDrive, iCloud Drive) for off-device
            redundancy.
          </div>
        ) : (
        ...
        <p className="text-xs text-muted-foreground">
          DeepDive also writes after data changes (debounced 60 s) so your latest progress is
