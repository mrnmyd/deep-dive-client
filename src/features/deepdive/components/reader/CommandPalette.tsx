...
import { searchTopics, type SearchRecord } from '@/features/deepdive/utils/search'
import { useSettings } from '@/features/deepdive/hooks/useDeepDive'
import {
  getDeepDiveBackup,
  resetDeepDiveStorage,
  restoreDeepDiveBackup,
} from '@/features/deepdive/utils/storage'
import { migrate } from '@/features/deepdive/utils/migrations'
...
const importData = () => {
  ...
  input.onchange = async () => {
    ...
    try {
      ...
      restoreDeepDiveBackup(result.backup)
...
const resetData = () => {
  if (!window.confirm('Reset all DeepDive progress in this browser? Cannot be undone.')) return
  resetDeepDiveStorage()
