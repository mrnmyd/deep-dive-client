...
import type { BackupConfig } from '@/features/preptracker/types/preptracker.types'
import {
  ...
  sha256Hex,
  writeBackupFile,
} from '@/features/preptracker/utils/auto-backup'
import {
  clearBackupHandle,
  loadBackupHandle,
  saveBackupHandle,
} from '@/features/preptracker/utils/idb'
import {
  defaultBackupConfig,
...
export function useAutoBackup() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('preptracker-storage', onChange)
    return () => window.removeEventListener('preptracker-storage', onChange)
