...
import { useDailyLog, useProblemTracker, useSettings, useTopicProgress } from "@/features/preptracker/hooks/usePrepTracker"
import { PageHeader, ProgressBar } from "@/features/preptracker/components/ui-helpers"
import { allTopics } from "@/features/preptracker/utils/progress"
import { getAllModules } from "@/features/preptracker/utils/study-content"
...
export function DashboardPage() {
  ...
  return (
    <div>
      <PageHeader
        title={settings.userName ? `Welcome back, ${settings.userName}` : "PrepTracker Dashboard"}
      ...
      {!settings.userName && (
        <Card className="mb-6">
          ...
          <CardContent className="grid gap-3 sm:grid-cols-[1fr_180px_auto]">
            <Input placeholder="Your name" onChange={(event) => updateSettings({ userName: event.target.value })} />
            <Input
              type="number"
              defaultValue={settings.dailyGoalMins}
              onChange={(event) => updateSettings({ dailyGoalMins: Number(event.target.value) || 180 })}
            />
            <Button asChild>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Flame className="h-4 w-4" /> Streak</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><Target className="h-4 w-4" /> Today</CardTitle>
          </CardHeader>
          <CardContent>
            <p className="text-3xl font-semibold">{dailyPercent}%</p>
            <ProgressBar value={dailyPercent} className="mt-3" />
            <p className="mt-2 text-sm text-muted-foreground">{dailyLog.todayLog.minutesStudied} / {settings.dailyGoalMins} minutes</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
          ...
          <CardContent>
            ...
            <p className="text-sm text-muted-foreground">Solved of {problemTracker.problems.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2"><BookOpen className="h-4 w-4" /> Study Modules</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]">
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2"><CalendarCheck className="h-4 w-4" /> Today's Session</CardTitle>
            </CardHeader>
            <CardContent>
              <p className="text-sm text-muted-foreground">
                {dailyLog.todayLog.dsaDone && dailyLog.todayLog.theoryDone && dailyLog.todayLog.revisionDone && dailyLog.todayLog.buildDone
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {recentTopics.length ? recentTopics.map(({ topic, updatedAt }) => (
                <div key={topic.id} className="rounded-lg border p-3">
                  <p className="font-medium">{topic.title}</p>
                  <p className="text-xs text-muted-foreground">{new Date(updatedAt).toLocaleString()}</p>
                </div>
